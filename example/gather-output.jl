...
function gather(output_dir)
    cd(output_dir)
    
    # Source database containing experiment metadata
    if !ispath("sweep_db.sqlite")
        error("`sweep_db.sql` does not exist; please run the sweep first")
    end
    ...
    for (run_id, run_dir) in run_pairs
        ...
        for (_,) in execute(db, "SELECT 1 FROM rundb.sqlite_master WHERE type = 'table' AND tbl_name='meta'")
