...
function main()
    ...
    execute(compdb, "CREATE TABLE comparisons (comparison_name, combo_id, ks_pvalue, frac_diff, mean1, mean2, mad1, mad2)")
    ...
    function do_comparison(comparison_name, q1; q2 = q1)
        for (combo_id, (test, (mean1, mad1), (mean2, mad2))) in compare(db1, db2, q1; q2 = q2)
            ...
            execute(comp_insert, [comparison_name, combo_id, pval, frac_diff, mean1, mean2, mad1, mad2])
        end
    end
    ...
    year_start = 90
    year_end = 100
    time_predicate_burnin = ("(time = $(year_burnin) * 360)")
    ...
    do_comparison(
        "n_infected_active_burnin",
        "SELECT n_infected_active FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )
    ...
    do_comparison(
        "n_infections_active_burnin",
        "SELECT n_infections_active FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )
    ...
    do_comparison(
        "n_bites_burnin",
        "SELECT n_bites FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )

    do_comparison(
        "n_infected_bites_burnin",
        "SELECT n_infected_bites FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )

    do_comparison(
        "n_infected_bites_with_space_burnin",
        "SELECT n_infected_bites_with_space FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )

    do_comparison(
        "n_transmitting_bites_burnin",
        "SELECT n_transmitting_bites FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )

    do_comparison(
        "n_transmissions_burnin",
        "SELECT n_transmissions FROM summary WHERE run_id = ? AND $(time_predicate_burnin)"
    )
    ...
    do_comparison(
        "n_circulating_genes_blood_burnin",
        "SELECT n_circulating_genes_blood FROM gene_strain_counts WHERE run_id = ? AND $(time_predicate_burnin)"
    )
    ...
    do_comparison(
        "n_circulating_strains_blood_burnin",
        "SELECT n_circulating_genes_blood FROM gene_strain_counts WHERE run_id = ? AND $(time_predicate_burnin)"
    )
